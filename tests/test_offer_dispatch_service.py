...
def test_dispatch_fallbacks_when_selector_empty() -> None:
    """Fallback to confirmed profiles if selector returns empty."""

    user_repo = InMemoryUserRepository()
    blogger_repo = InMemoryBloggerProfileRepository()
    order_repo = InMemoryOrderRepository()

    class EmptySelector(BloggerRelevanceSelector):
        """Selector that returns no matches."""
    ...
    service = OfferDispatchService(
        ...
        blogger_repo=blogger_repo,
        order_repo=order_repo,
        relevance_selector=EmptySelector(),
    )

    order = Order(
        order_id=UUID("00000000-0000-0000-0000-000000000630"),
        advertiser_id=UUID("00000000-0000-0000-0000-000000000631"),
        product_link="https://example.com",
        offer_text="Offer",
        ugc_requirements=None,
        barter_description=None,
    ...
    blogger = User(
        user_id=UUID("00000000-0000-0000-0000-000000000632"),
        external_id="105",
        messenger_type=MessengerType.TELEGRAM,
        username="blogger",
        role=UserRole.BLOGGER,
