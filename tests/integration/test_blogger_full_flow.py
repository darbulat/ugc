...
async def test_blogger_full_flow(
    ...
    """Test blogger flow: registration → Instagram → offer → feedback."""
    ...
    # === Step 2: Instagram verification (generate_code only) ===
    ...
    # Skip verify_code: needs blogger_profiles (JSONB, not in SQLite)
    ...
    # === Step 4: Payment and order activation (no outbox table) ===
    ...
    assert (
        await user_repo.get_by_id(blogger_user.user_id, session=session)
        is not None
    )
    assert (
        await user_repo.get_by_id(advertiser_user.user_id, session=session)
        is not None
    )
    assert (
        await order_repo.get_by_id(order.order_id, session=session) is not None
    )
