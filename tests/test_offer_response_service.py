...
def test_offer_response_with_metrics() -> None:
    """Record metrics when metrics collector is provided."""
    ...
    class MockMetricsCollector:
        ...
        def record_blogger_response(self, order_id: str, blogger_id: str) -> None:
            """Record response."""
            self.recorded_order_id = order_id
            self.recorded_blogger_id = blogger_id
    ...
    response = service.respond(order_id=order.order_id, blogger_id=blogger_id)

    assert response.order_id == order.order_id
    assert metrics_collector.recorded_order_id == str(order.order_id)
    assert metrics_collector.recorded_blogger_id == str(blogger_id)
