...
class SqlAlchemyUserRepository(UserRepository):
        ...
        with self.session_factory() as session:
            result = session.execute(
                select(UserModel).where(
                    ...
                    UserModel.messenger_type == messenger_type,
                )
            ).scalar_one_or_none()
            return _to_user_entity(result) if result else None
    ...
    def get_by_instagram_url(self, instagram_url: str) -> Optional[User]:
        ...
        with self.session_factory() as session:
            result = session.execute(
                select(UserModel).where(UserModel.instagram_url == instagram_url)
            ).scalar_one_or_none()
            return _to_user_entity(result) if result else None

    def list_confirmed_by_role(self, role: UserRole) -> list[User]:
        """List users with confirmed Instagram and matching role."""

        roles = [role]
        if role != UserRole.BOTH:
            roles.append(UserRole.BOTH)
        with self.session_factory() as session:
            results = session.execute(
                select(UserModel).where(
                    UserModel.confirmed.is_(True),
                    UserModel.role.in_(roles),
                )
            ).scalars()
            return [_to_user_entity(item) for item in results]
...
def _to_user_entity(model: UserModel) -> User:
    ...
    return User(
        ...
        profile_updated_at=model.profile_updated_at,
    )
